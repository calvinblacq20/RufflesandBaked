export function receiptNumber(year: number, sequence: number): string {
  return `RBHR-${year}-${String(sequence).padStart(4, "0")}`;
}

export function orderNumber(sequence: number): string {
  return `RBH-${String(sequence).padStart(4, "0")}`;
}
...
/** Short check code printed on receipts so the bakery can match them to its records. */
