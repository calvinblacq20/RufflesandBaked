...
export function AccountSheet({ open, onClose, mode = "login", defaultName = "", defaultPhone = "", onDone }: {
  ...
  const verified = () => {
    if (current === "login") {
      ...
      notify("Logged in", `Welcome back, ${customer?.name.split(" ")[0] ?? "there"}. Your orders and dates are here.`);
    } else {
      const customer = actions.createAccount({ name: name.trim(), phone: formatGhPhone(phone) });
      notify("Account created", `Your orders and dates are saved to ${formatGhPhone(customer.phone)}.`);
    }
    onClose();
    onDone?.();
  };
        ...
        >
          <p className="muted">
            {current === "login"
              ...
              : "Optional. Keep your orders, birthday reminders and loyalty points on any phone. We'll confirm your WhatsApp number with a code."}
          </p>
          {current === "create" && (
            <div className="field">
...
export function FindOrderSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const submit = () => {
    const next: typeof errors = {};
    if (!normalizeOrderNumber(number)) next.number = "Enter the order number from your WhatsApp message, like RBH-1041.";
    if (!normalizeGhPhone(phone)) next.phone = PHONE_ERROR;
    if (!next.number && !next.phone) {
      ...
      if (order) {
        setOrderId(order.id);
        setStage("code");
      } else next.match = "We couldn't find an order with that order number and WhatsApp number. Check both, or message the bakery.";
    }
    setErrors(next);
  };
      ...
      {stage === "code" && orderId ? (
        ...
        >
          ...
          <div className="field">
            ...
            <input id="find-number" autoCapitalize="characters" placeholder="RBH-1041" value={number} onChange={(e) => setNumber(e.target.value)} aria-invalid={Boolean(errors.number)} aria-describedby="find-number-hint" style={{ fontFamily: "var(--mono)" }} />
            {errors.number && (
